@if(!empty($seo))
    ...
    <meta itemprop="image" content="{{ get_image(config('constants.frontend.seo.path') .'/'. $seo->image) }}">

    <!-- Facebook Meta Tags -->
    <meta property="og:type" content="website">
    <meta property="og:title" content="{{ $seo->social_title }}">
    <meta property="og:description" content="{{ $seo->social_description }}">
    <meta property="og:image" content="{{ get_image(config('constants.frontend.seo.path') .'/'. $seo->image) }}"/>
    <meta property="og:image:type" content="image/{{ pathinfo(get_image(config('constants.frontend.seo.path') .'/'. $seo->image))['extension'] }}" />
